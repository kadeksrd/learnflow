...
export function CourseCard({ product }: CourseCardProps) {
  ...
  return (
    <Link href={`/course/${slug}`} className="group">
      <div className="bg-card border border-white/[0.07] rounded-2xl overflow-hidden hover:border-accent/50 hover:-translate-y-1 hover:shadow-2xl hover:shadow-accent/10 transition-all duration-300 h-full flex flex-col">
        {/* Thumbnail */}
        <div className="relative h-44 overflow-hidden bg-surface shrink-0">
          {product.thumbnail ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-5xl"
              style={{ background: `linear-gradient(135deg, hsl(${product.id.charCodeAt(0) * 3 % 360}, 40%, 12%), hsl(${product.id.charCodeAt(1) * 3 % 360}, 50%, 20%))` }}>
          ...
          {product.categories && (
            <span className="absolute top-3 left-3 px-2.5 py-1 bg-black/50 backdrop-blur-sm text-text-muted text-xs rounded-full">
        ...
        <div className="p-5 flex flex-col flex-1">
          <h3 className="font-syne font-bold text-sm leading-snug mb-3 line-clamp-2 group-hover:text-accent-light transition-colors flex-1">
            {product.title}
          </h3>
          ...
          <div className="flex items-center justify-between mt-auto">
            <span className="font-syne font-extrabold text-lg">
              {product.is_free ? <span className="text-green-400">GRATIS</span> : <span>{formatPrice(product.price)}</span>}
            </span>
            <span className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
              product.is_free
                ? 'bg-green-500/10 text-green-400 border border-green-500/20 group-hover:bg-green-500 group-hover:text-white'
