...
async function getStoreData() {
  ...
  const [
    ...
    supabase
      .from('products')
      .select('*, categories(id,name,slug), landing_pages(slug), courses(id, modules(id, lessons(id)))')
      .eq('is_published', true)
      .order('created_at', { ascending: false }),
    supabase.from('categories').select('*').order('name'),
    supabase.from('site_settings').select('value').eq('key', 'store').single(),
    supabase.from('site_settings').select('value').eq('key', 'seo_global').single(),
  ])
...
export async function generateMetadata(): Promise<Metadata> {
  const supabase = await createClient()
  const [{ data: storeSetting }, { data: seoGlobal }] = await Promise.all([
    supabase.from('site_settings').select('value').eq('key', 'store').single(),
    supabase.from('site_settings').select('value').eq('key', 'seo_global').single(),
  ])

  const store = storeSetting?.value || {}
  const seo   = seoGlobal?.value   || {}
  const siteName = seo.site_name || 'LearnFlow'

  const title       = `Semua Kursus | ${siteName}`
  const description = store.subheadline || `Temukan kursus digital terbaik di ${siteName}. Belajar kapan saja, di mana saja.`
...
export default async function StorePage() {
  ...
  const subheadline     = store.subheadline      || `${products.length} kursus dari instruktur terpercaya`
    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="relative overflow-hidden border-b border-white/[0.05]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-14">
          ...
          <p className="text-text-muted text-sm sm:text-base mb-6 sm:mb-8 max-w-xl">{subheadline}</p>

          {show_stats && (
            <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-1" style={{ scrollbarWidth:'none' }}>
              {[
                { icon: BookOpen, value: `${products.length}+`, label: 'Total Kursus'    },
                { icon: Sparkles, value: `${freeCount}`,        label: 'Kursus Gratis'   },
                { icon: BookOpen, value: `${paidCount}`,        label: 'Kursus Premium'  },
