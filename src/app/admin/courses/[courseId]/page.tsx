...
export default async function CourseBuilderPage({ params }: { params: { courseId: string } }) {
  ...
  return (
    <div className="p-4 sm:p-8">
      {/* Back */}
      <div className="mb-6">
        <Link href="/admin/courses" className="flex items-center gap-1.5 text-sm text-text-muted hover:text-[#EEEEFF] transition-colors w-fit mb-4">
          <ArrowLeft size={14} /> Kembali ke Daftar Kursus
        </Link>
        <h1 className="font-syne font-extrabold text-xl sm:text-2xl mb-1">Course Builder</h1>
        <p className="text-text-muted text-sm">{(course.products as any)?.title}</p>
      </div>
