...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await s.from('products').insert(body).select().single() as any
  if (error) return NextResponse.json({ message: error.message }, { status: 500 })

  // Auto-create course linked to this product
  if (data) {
    await s.from('courses').insert({ product_id: data.id, title: data.title } as any)
  }

  // Return with flag so frontend can redirect to landing page editor
  return NextResponse.json({ ...(data as any), redirect_to_landing: true }, { status: 201 })
}
