...
export default async function DashboardPage() {
  ...
  const completedSet = new Set(
    (progressData as any)?.map((p: any) => p.lesson_id) || [],
  );

  // Today's progress (lessons completed today)
  const today = new Date().toISOString().slice(0, 10);
  const todayCount =
    (progressData as any)?.filter((p: any) => p.completed_at?.startsWith(today))
      .length || 0;
  ...
  for (const uc of enrolled || []) {
    const course = (uc as any).courses;
    if (!course) continue;
  ...
  const reviewMap = new Map(
    (existingReviews as any)?.map((r: any) => [r.product_id, r]) || [],
  );
