...
import { BookOpen, Star, Target, Play, ChevronRight, Flame, Award, Clock } from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  const { data: enrolled } = await supabase
    .from('user_courses')
    .select(`enrolled_at, courses(id, title, products(id, title, thumbnail, categories(name)), modules(id, lessons(id, title, duration, video_url)))`)
    .eq('user_id', user!.id)
    .order('enrolled_at', { ascending: false })

  const allLessonIds = (enrolled || []).flatMap((uc: any) =>
    uc.courses?.modules?.flatMap((m: any) => m.lessons.map((l: any) => l.id)) || []
  )
  ...
  // Reviews
  const productIds = (enrolled || []).map((uc: any) => uc.courses?.products?.id).filter(Boolean)
    ...
    <div className="min-h-screen bg-bg">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            <h1 className="font-syne font-extrabold text-2xl sm:text-3xl mb-1">
              {todayCount > 0 ? `Semangat, ${userName}! 🔥` : `Halo, ${userName}! 👋`}
            </h1>
            <p className="text-text-muted text-sm">
              {todayCount > 0
                ? `Kamu sudah menyelesaikan ${todayCount} lesson hari ini. Terus!`
                : nextLesson ? 'Lanjutkan belajar hari ini' : 'Mulai perjalanan belajarmu'}
            </p>
          </div>
          <Link href="/store" className="px-3 sm:px-4 py-2 bg-card border border-white/[0.07] rounded-xl text-xs sm:text-sm text-text-muted hover:text-[#EEEEFF] hover:border-accent/30 transition-all shrink-0 ml-3">
            + Kursus
          </Link>
        </div>
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-accent/20 via-accent/10 to-transparent border border-accent/30 rounded-2xl p-5 sm:p-6">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-accent/20 border border-accent/30 rounded-full">
                      ...
                      <span className="text-accent-light text-xs font-bold">Belajar Hari Ini</span>
                    </div>
                    {todayCount > 0 && (
                      <div className="flex items-center gap-1 px-2.5 py-1 bg-cta/15 border border-cta/25 rounded-full">
                        ...
                        <span className="text-cta text-xs font-bold">{todayCount} selesai</span>
                      </div>
                    )}
                  </div>
                  <p className="text-text-muted text-xs mb-1 font-medium">{nextCourse.products?.title}</p>
                  <h2 className="font-syne font-extrabold text-lg sm:text-xl mb-3 leading-tight">
                    {nextLesson.title}
                  </h2>
                  <div className="flex items-center gap-3 text-xs text-text-muted flex-wrap">
                    {nextLesson.duration > 0 && (
                      <span className="flex items-center gap-1"><Clock size={11} /> {Math.floor(nextLesson.duration / 60)} menit</span>
                    )}
                    <span className="flex items-center gap-1"><BookOpen size={11} /> {completedLessons}/{totalLessons} lesson selesai</span>
                  </div>
                </div>
                <Link href={`/dashboard/course/${nextCourse.id}`}
              ...
              <div className="relative mt-5 pt-4 border-t border-white/[0.07]">
                <div className="flex justify-between text-xs text-text-muted mb-2">
                  ...
                  <span className="font-semibold text-accent-light">{overallProgress}%</span>
                </div>
                <div className="h-2 bg-black/30 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-accent to-accent-light rounded-full transition-all"
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            { icon: BookOpen, value: enrolled?.length || 0, label: 'Kursus',   color: 'text-accent-light', bg: 'bg-accent/10' },
            { icon: Award,    value: completedLessons,       label: 'Lesson ✓', color: 'text-green-400',    bg: 'bg-green-500/10' },
            { icon: Star,     value: existingReviews?.length || 0, label: 'Review', color: 'text-cta',    bg: 'bg-cta/10' },
          ].map(({ icon: Icon, value, label, color, bg }) => (
            <div key={label} className="bg-card border border-white/[0.07] rounded-2xl p-3 sm:p-5">
              <div className={`w-8 h-8 sm:w-10 sm:h-10 ${bg} rounded-xl flex items-center justify-center mb-2 sm:mb-3`}>
                <Icon size={16} className={color} />
              </div>
              <div className={`font-syne font-extrabold text-xl sm:text-2xl ${color}`}>{value}</div>
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-syne font-bold text-lg sm:text-xl">Kursus Saya</h2>
        </div>
        ...
        {(enrolled?.length || 0) > 0 ? (
          <div className="space-y-4">
            {(enrolled || []).map((uc: any) => {
              ...
              const total    = course.modules?.reduce((s: number, m: any) => s + m.lessons.length, 0) || 0
              const done     = course.modules?.reduce((s: number, m: any) =>
                s + m.lessons.filter((l: any) => completedSet.has(l.id)).length, 0) || 0
              ...
              for (const mod of (course.modules || [])) {
                for (const lesson of (mod.lessons || [])) {
                  if (!completedSet.has(lesson.id)) { courseNextLesson = lesson; break }
                }
                if (courseNextLesson) break
              }

              return (
                <div key={course.id} className="bg-card border border-white/[0.07] rounded-2xl overflow-hidden hover:border-white/[0.12] transition-all">
                  <div className="flex flex-col sm:grid sm:grid-cols-[260px_1fr]">
                    <div className="p-4 sm:p-5 sm:border-r border-b sm:border-b-0 border-white/[0.07]">
                      <MyCourseCard course={course} progress={progress} completedLessons={done} totalLessons={total} />
                      ...
                      {courseNextLesson && progress < 100 && (
                        <Link href={`/dashboard/course/${course.id}`}
                          ...
                          <Play size={12} /> Lanjut: {courseNextLesson.title.slice(0, 28)}{courseNextLesson.title.length > 28 ? '...' : ''}
                        </Link>
                      )}
                    </div>
                      ...
                      {progress > 0 ? (
                        <>
                          <p className="text-xs text-text-muted mb-3 font-semibold uppercase tracking-wider">
                            {review ? '⭐ Rating kamu' : '💬 Bagikan pengalamanmu'}
                          </p>
                          {!review && (
                            <p className="text-xs text-text-dim mb-3 leading-relaxed">
                              Review kamu bisa tampil di halaman kursus dan membantu pelajar lain.
                            </p>
                          )}
                          {product?.id && <ReviewForm productId={product.id} existingReview={review || null} />}
                        </>
                      ) : (
                        <div className="flex flex-col items-center justify-center py-6 text-center h-full">
                          <div className="text-3xl mb-3">📚</div>
                          <p className="text-text-muted text-sm font-semibold mb-1">Mulai belajar dulu!</p>
                          <p className="text-text-dim text-xs mb-3">Selesaikan 1 lesson untuk beri rating.</p>
                          {courseNextLesson && (
                            <Link href={`/dashboard/course/${course.id}`}
          ...
          <div className="text-center py-12 sm:py-16 bg-card border border-white/[0.07] rounded-2xl">
            ...
            <h3 className="font-syne font-bold text-lg mb-2">Belum ada kursus</h3>
            <p className="text-text-muted text-sm mb-6">Mulai belajar dengan memilih kursus favoritmu</p>
            <Link href="/store" className="inline-flex items-center gap-2 px-5 py-3 bg-accent rounded-xl text-sm font-bold hover:bg-accent-light transition-all">
              Explore Kursus →
            </Link>
          </div>
        )}
